/*
 * CrossID Public API
 *
 * CrossID API Reference 
 *
 * API version: 1.0.0
 * Contact: developer@crossid.io
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cidclient

import (
	"encoding/json"
)

// FormButtonAttributes struct for FormButtonAttributes
type FormButtonAttributes struct {
	Name string `json:"name"`
	Role *string `json:"role,omitempty"`
	Type string `json:"type"`
	Value string `json:"value"`
}

// NewFormButtonAttributes instantiates a new FormButtonAttributes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFormButtonAttributes(name string, type_ string, value string) *FormButtonAttributes {
	this := FormButtonAttributes{}
	this.Name = name
	this.Type = type_
	this.Value = value
	return &this
}

// NewFormButtonAttributesWithDefaults instantiates a new FormButtonAttributes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFormButtonAttributesWithDefaults() *FormButtonAttributes {
	this := FormButtonAttributes{}
	return &this
}

// GetName returns the Name field value
func (o *FormButtonAttributes) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *FormButtonAttributes) GetNameOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *FormButtonAttributes) SetName(v string) {
	o.Name = v
}

// GetRole returns the Role field value if set, zero value otherwise.
func (o *FormButtonAttributes) GetRole() string {
	if o == nil || o.Role == nil {
		var ret string
		return ret
	}
	return *o.Role
}

// GetRoleOk returns a tuple with the Role field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FormButtonAttributes) GetRoleOk() (*string, bool) {
	if o == nil || o.Role == nil {
		return nil, false
	}
	return o.Role, true
}

// HasRole returns a boolean if a field has been set.
func (o *FormButtonAttributes) HasRole() bool {
	if o != nil && o.Role != nil {
		return true
	}

	return false
}

// SetRole gets a reference to the given string and assigns it to the Role field.
func (o *FormButtonAttributes) SetRole(v string) {
	o.Role = &v
}

// GetType returns the Type field value
func (o *FormButtonAttributes) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *FormButtonAttributes) GetTypeOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *FormButtonAttributes) SetType(v string) {
	o.Type = v
}

// GetValue returns the Value field value
func (o *FormButtonAttributes) GetValue() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *FormButtonAttributes) GetValueOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *FormButtonAttributes) SetValue(v string) {
	o.Value = v
}

func (o FormButtonAttributes) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["name"] = o.Name
	}
	if o.Role != nil {
		toSerialize["role"] = o.Role
	}
	if true {
		toSerialize["type"] = o.Type
	}
	if true {
		toSerialize["value"] = o.Value
	}
	return json.Marshal(toSerialize)
}

type NullableFormButtonAttributes struct {
	value *FormButtonAttributes
	isSet bool
}

func (v NullableFormButtonAttributes) Get() *FormButtonAttributes {
	return v.value
}

func (v *NullableFormButtonAttributes) Set(val *FormButtonAttributes) {
	v.value = val
	v.isSet = true
}

func (v NullableFormButtonAttributes) IsSet() bool {
	return v.isSet
}

func (v *NullableFormButtonAttributes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFormButtonAttributes(val *FormButtonAttributes) *NullableFormButtonAttributes {
	return &NullableFormButtonAttributes{value: val, isSet: true}
}

func (v NullableFormButtonAttributes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFormButtonAttributes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


